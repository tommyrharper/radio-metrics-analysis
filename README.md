...
**Compare papers:** Each paper row has **Compare** (toggle; max 5). A sticky tray shows the selection; **Compare papers** opens [`site/compare.html`](site/compare.html). Two views: **Overview** (category counts + metric presence matrix) and **Detailed** (side-by-side structured subtypes). Cohort checkboxes also persist via `site/js/cohort-prefs.js`.
