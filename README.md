...
**Compare papers:** Each paper row has **Compare** (toggle; max 5). A sticky tray shows the selection; **Compare papers** opens [`site/compare.html`](site/compare.html) for side-by-side structured metric details (metric filter + per-paper columns). Selection is stored in `localStorage` via `site/js/comparison-store.js`. Cohort checkboxes (Classic / Emerging-ML / R2D2-citing) also persist across pages via `site/js/cohort-prefs.js`.
